...
impl DynExpr for Box<dyn DynExpr> {
    fn render_dyn(&self, buf: &mut String) { (**self).render_dyn(buf); }
}
...
impl<E: DynExpr> Expr for E {
    fn ty_hint(&self) -> &'static str { "any" }
}
...
impl SurrealQL for bool {
    fn surreal_type() -> &'static str { "bool" }
    fn render_literal(value: &Self, buf: &mut String) {
...
macro_rules! surreal_display {
    ($t:ty, $name:literal) => {
        impl SurrealQL for $t {
            fn surreal_type() -> &'static str { $name }
            fn render_literal(value: &Self, buf: &mut String) {
                use std::fmt::Write;
...
impl SurrealQL for chrono::DateTime<chrono::Utc> {
    fn surreal_type() -> &'static str { "datetime" }
    fn render_literal(value: &Self, buf: &mut String) {
        buf.push('\'');
...
impl SurrealQL for uuid::Uuid {
    fn surreal_type() -> &'static str { "uuid" }
    fn render_literal(value: &Self, buf: &mut String) {
        use std::fmt::Write;
        buf.push('\'');
        let _ = write!(buf, "{value}");
...
impl<T: SurrealQL> SurrealQL for Option<T> {
    fn surreal_type() -> &'static str { T::surreal_type() }
    fn render_literal(value: &Self, buf: &mut String) {
        match value {
...
impl<T: crate::types::SurrealRecord> SurrealQL for crate::types::Thing<T> {
    fn surreal_type() -> &'static str { "record" }
    fn render_literal(value: &Self, buf: &mut String) {
        use std::fmt::Write;
...
impl<V: SurrealQL> DynExpr for Literal<V> {
    fn render_dyn(&self, buf: &mut String) { V::render_literal(&self.0, buf); }
}
...
impl<T: SurrealRecord, V: SurrealQL> DynExpr for Column<T, V> {
    fn render_dyn(&self, buf: &mut String) { buf.push_str(self.name); }
}
...
pub fn ident(name: &'static str) -> Ident { Ident(name) }

impl Ident {
    fn dyn_box(&self) -> Box<dyn DynExpr> { Box::new(Raw(self.0.to_string())) }

    pub fn eq<V: SurrealQL>(&self, v: V) -> EqExpr {
        EqExpr { left: self.dyn_box(), right: Box::new(Literal(v)) }
    }
    pub fn ne<V: SurrealQL>(&self, v: V) -> NeExpr {
        NeExpr { left: self.dyn_box(), right: Box::new(Literal(v)) }
    }
    pub fn gt<V: SurrealQL>(&self, v: V) -> GtExpr {
        GtExpr { left: self.dyn_box(), right: Box::new(Literal(v)) }
    }
    pub fn lt<V: SurrealQL>(&self, v: V) -> LtExpr {
        LtExpr { left: self.dyn_box(), right: Box::new(Literal(v)) }
    }
    pub fn gte<V: SurrealQL>(&self, v: V) -> GteExpr {
        GteExpr { left: self.dyn_box(), right: Box::new(Literal(v)) }
    }
    pub fn lte<V: SurrealQL>(&self, v: V) -> LteExpr {
        LteExpr { left: self.dyn_box(), right: Box::new(Literal(v)) }
    }
    pub fn contains<V: SurrealQL>(&self, v: V) -> ContainsExpr {
        ContainsExpr { haystack: self.dyn_box(), needle: Box::new(Literal(v)) }
    }
    /// Compare to an arbitrary expression — e.g. `asset = type::record('asset', …)`.
    pub fn eq_expr(&self, rhs: impl DynExpr + 'static) -> EqExpr {
        EqExpr { left: self.dyn_box(), right: Box::new(rhs) }
    }
    pub fn ne_expr(&self, rhs: impl DynExpr + 'static) -> NeExpr {
        NeExpr { left: self.dyn_box(), right: Box::new(rhs) }
    }
    /// `field IS NONE`.
    pub fn is_none(&self) -> Raw { Raw(format!("{} IS NONE", self.0)) }
}

impl DynExpr for Ident {
    fn render_dyn(&self, buf: &mut String) { buf.push_str(self.0); }
}
...
impl DynExpr for NoneLit {
    fn render_dyn(&self, buf: &mut String) { buf.push_str("NONE"); }
}
...
impl RecordLink {
    ...
    pub fn new<V: SurrealQL>(table: &'static str, key: V) -> Self {
        Self { table, key: Box::new(Literal(key)) }
    }
...
impl Func {
    ...
    pub fn of(name: &'static str, ident: &'static str) -> Self {
        Self { name, args: vec![Box::new(Raw(ident.to_string()))] }
    }
}
...
impl<T: SurrealRecord, V: SurrealQL> Column<T, V> {
    pub fn eq(&self, value: V) -> EqExpr {
        EqExpr { left: self.dyn_box(), right: Box::new(Literal(value)) }
    }
    pub fn ne(&self, value: V) -> NeExpr {
        NeExpr { left: self.dyn_box(), right: Box::new(Literal(value)) }
    }
    pub fn gt(&self, value: V) -> GtExpr {
        GtExpr { left: self.dyn_box(), right: Box::new(Literal(value)) }
    }
    pub fn lt(&self, value: V) -> LtExpr {
        LtExpr { left: self.dyn_box(), right: Box::new(Literal(value)) }
    }
    pub fn gte(&self, value: V) -> GteExpr {
        GteExpr { left: self.dyn_box(), right: Box::new(Literal(value)) }
    }
    pub fn lte(&self, value: V) -> LteExpr {
        LteExpr { left: self.dyn_box(), right: Box::new(Literal(value)) }
    }
    pub fn contains(&self, value: V) -> ContainsExpr {
        ContainsExpr { haystack: self.dyn_box(), needle: Box::new(Literal(value)) }
    }
    ...
    fn dyn_box(&self) -> Box<dyn DynExpr> {
        Box::new(Self { name: self.name, surreal_type: self.surreal_type, _marker: self._marker })
    }
}
...
combinators!(EqExpr, NeExpr, GtExpr, LtExpr, GteExpr, LteExpr, AndExpr, OrExpr, ContainsExpr, NotExpr, Raw);
...
impl Grouped {
    pub fn new(inner: impl DynExpr + 'static) -> Self { Self(Box::new(inner)) }
}
...
impl Projection {
    ...
    pub fn new(expr: impl DynExpr + 'static) -> Self {
        Self { expr: Box::new(expr), alias: None }
    }
    /// `<expr> AS <alias>`.
    pub fn aliased(expr: impl DynExpr + 'static, alias: &'static str) -> Self {
        Self { expr: Box::new(expr), alias: Some(alias) }
    }
...
impl<T: SurrealRecord> std::fmt::Debug for ColumnSet<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ColumnSet").field("cols", &self.cols).finish()
    }
}

impl<T: SurrealRecord> DynExpr for ColumnSet<T> {
    fn render_dyn(&self, buf: &mut String) { buf.push('*'); }
}
